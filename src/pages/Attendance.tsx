...
const ATTENDANCE_COLUMNS = "id, check_in_time, check_out_time, late_minutes, early_minutes, deduction_applied";
...
function parseSettingsRows(rows: { key: string; value: string }[] | null): Settings {
...
export default function Attendance() {
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      const [attRes, settRes, salRes, profileRes, evRes, leaveRes] = await Promise.all([
        supabase.from("attendance").select(ATTENDANCE_COLUMNS).eq("user_id", user.id).eq("date", today).maybeSingle(),
        supabase
          .from("app_settings")
          .select("key, value")
        ...
        supabase
          .from("profiles")
          .select("role, full_name, work_day, check_in_time, check_out_time, work_schedule, base_salary")
          .eq("id", user.id)
          .maybeSingle(),
        supabase
      ...
      if (profileRes.data) {
        const p = profileRes.data as {
          ...
          work_schedule?: Record<string, { active: boolean; check_in: string; check_out: string }>;
          base_salary?: number;
        };
        ...
        setProfileBaseSalary(p.base_salary ?? 300000);
      }

      if (settRes.data) {
        setSettings(parseSettingsRows(settRes.data as { key: string; value: string }[]));
      }

      const approvedLeaves =
        (leaveRes.data as { type: string; start_time: string | null; end_time: string | null }[]) ?? [];
      setApprovedLeaveTypesToday(approvedLeaves.map((l) => l.type));
      setHasFullLeaveToday(approvedLeaves.some((l) => l.type === "leave" && !l.start_time && !l.end_time));

      const holidays = (evRes.data as { id: string; assigned_to_all: boolean }[]) ?? [];
      const needsAssignmentCheck = holidays.some((e) => !e.assigned_to_all);
      let myEventIds = new Set<string>();
      if (needsAssignmentCheck && holidays.length > 0) {
        ...
        myEventIds = new Set((assignRows ?? []).map((r) => r.event_id));
      }
      setIsHolidayToday(holidays.some((e) => e.assigned_to_all || myEventIds.has(e.id)));
    } catch (e) {
  ...
  const requestLocationPermission = (): Promise<LocationState> => {
    return new Promise((resolve) => {
      ...
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          const next: LocationState = { status: "granted", lat, lng, distance, isInside, errorMessage: null };
          setLocation(next);
          resolve(next);
        },
        (err) => {
  ...
  const getSalaryForDisplay = (): SalaryRecord => {
    if (salary) return salary;
    return {
      base_salary: profileBaseSalary,
      current_salary: profileBaseSalary,
      total_deductions: 0,
    };
  };
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      if (schoolConfigured && loc.status !== "granted") {
        loc = await requestLocationPermission();
        if (loc.status !== "granted" && !isAdmin) {
          toast({
      ...
      if (schoolConfigured && loc.isInside === false && !isAdmin) {
        toast({
          title: "Outside school area",
          description: `You are ${loc.distance}m away. Move closer to check in.`,
          variant: "destructive",
        });
        return;
      }
      if (schoolConfigured && (loc.status === "denied" || loc.status === "error") && !isAdmin) {
        toast({
          title: "Location permission is required",
      ...
      if (loc.lat != null) {
        insertData.check_in_lat = loc.lat;
        insertData.check_in_lng = loc.lng;
        insertData.check_in_distance = loc.distance;
      }

      const { data, error } = await supabase.from("attendance").insert(insertData).select(ATTENDANCE_COLUMNS).single();
      ...
      if (error) {
        ...
        const sal = getSalaryForDisplay();
        setSalary(sal);
        const overrideNote = (loc.status === "denied" || loc.status === "error") && isAdmin ? " (Admin override)" : "";
        toast({
          title:
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      if (schoolConfigured && locationRef.current.status !== "granted") {
      ...
      const hasApprovedLeave = approvedLeaveTypesToday.includes("leave");
      const hasApprovedLateExcuse = approvedLeaveTypesToday.includes("late_excuse");
          ...
          return (
            <Card className="border-l-4 border-l-destructive border border-border bg-destructive/5 shadow-none">
              <CardContent className="p-4 text-sm leading-relaxed">
                <p>
                  <span className="font-semibold">{displayName}</span> ယနေ့ သင့်အားလပ်ရက် ဖြစ်ပါတယ် အိပ်စရာတွေ ရှိတာတွေ
                  လုပ်စရာ ရှိတာတွေ သွားစရာ ရှိတာတွေ ကို သတိလေးထားပြီး ပျော်ပျော်ရွှင်ရွှင် လှုပ်ရှား လုပ်ကိုင်
                  သွားလာနိုင်ပါစေရှင့်။
                </p>
              </CardContent>
            </Card>
          );
