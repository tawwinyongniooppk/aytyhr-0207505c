...
BEGIN
  IF TG_TABLE_NAME = 'tasks' THEN
    IF NEW.assignee_id IS NULL OR NEW.due_date IS NULL THEN
      RETURN NEW;
    END IF;
    v_new_start := COALESCE((NEW.created_at AT TIME ZONE 'Asia/Yangon')::date, ((now() AT TIME ZONE 'Asia/Yangon'))::date);
    ...
    IF EXISTS (
      ...
      WHERE t.assignee_id = NEW.assignee_id
        ...
        AND daterange(COALESCE((t.created_at AT TIME ZONE 'Asia/Yangon')::date, t.due_date), t.due_date, '[]') && daterange(v_new_start, v_new_end, '[]')
    ) THEN
