...
BEGIN
  ...
  DELETE FROM public.salary_manual_deductions
  WHERE month < cutoff AND source = 'partial_leave';
END;
$function$;
...
BEGIN
  ...
  END IF;
  new_period_start := new_period::timestamp AT TIME ZONE 'Asia/Yangon';
  ...
  DELETE FROM public.leave_manual_deductions WHERE created_at < new_period_start;
