...
BEGIN
  ...
  END IF;
  -- C-6.1: no backward regression out of a submitted state by the assignee.
  IF NEW.submission_status IS DISTINCT FROM OLD.submission_status
     AND OLD.submission_status = 'submitted'
     AND NEW.submission_status IN ('not_started','not_submitted','in_progress','overdue') THEN
    RAISE EXCEPTION 'INVALID_TRANSITION: a submitted task cannot be moved back';
  END IF;
  -- C-6: block acknowledge/submit once the deadline date has passed (Asia/Yangon, date-only).
  IF NEW.submission_status IS DISTINCT FROM OLD.submission_status
     AND NEW.submission_status IN ('in_progress','submitted')
     AND NEW.due_date IS NOT NULL
     AND NEW.due_date < (now() AT TIME ZONE 'Asia/Yangon')::date THEN
    RAISE EXCEPTION 'DEADLINE_PASSED: Deadline exceeded; this task can no longer be updated';
  END IF;
  RETURN NEW;
END;
$function$;
...
BEGIN
  ...
  IF NEW.submission_status IS DISTINCT FROM OLD.submission_status
     AND OLD.submission_status = 'submitted'
     AND NEW.submission_status IN ('not_started','not_submitted','in_progress','overdue') THEN
    RAISE EXCEPTION 'INVALID_TRANSITION: a submitted assignment cannot be moved back';
  END IF;
  -- C-6: block acknowledge/submit once the deadline date has passed (Asia/Yangon, date-only).
  IF NEW.submission_status IS DISTINCT FROM OLD.submission_status
     AND NEW.submission_status IN ('in_progress','submitted') THEN
